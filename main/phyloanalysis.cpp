...
void reportAlignment(ofstream &out, Alignment &alignment, int nremoved_seqs) {
    ...
    case SEQ_TEA: out << "TEA structural"; break;
...
void reportModel(ostream &out, Alignment *aln, ModelSubst *m) {
    ...
    if (m->num_states <= 4) {
        ...
        if (m->isReversible()) {
            // Q is normalised to 1 subst per unit time; Q_ji/pi_i gives the exchangeability
            m->getQMatrix(full_mat);
            ...
            for (i = 1; i < m->num_states; i++) {
                for (j = 0; j < i; j++)
                    out << " " << (state_freq[i] > 0.0 ? full_mat[j*m->num_states+i] / state_freq[i] : 0.0);
                out << endl;
            }
        } else {
            // non-reversible model
